...
## Важно: Для сборки этого проекта необходимо установить OpenSSL

Ваша система Windows в настоящее время **не имеет установленного OpenSSL**, который необходим для сборки CryptoCore. Следуйте приведенным ниже инструкциям, чтобы установить необходимые зависимости и собрать проект.

---

## Рекомендуется: Установите MSYS2 (Самый простой метод)

MSYS2 предоставляет Unix-подобное окружение на Windows с простым управлением пакетами.
...
1. Убедитесь, что используете терминал **MSYS2 MINGW64** (не обычный MSYS2 или PowerShell)
2. Проверьте, что GCC установлен: `gcc --version`
3. Проверьте, что OpenSSL установлен: `pacman -Qs openssl`
4. Проверьте, что все зависимости установлены: `pacman -S mingw-w64-x86_64-gcc mingw-w64-x86_64-openssl make`

Для помощи, специфичной для Windows, см. [WINDOWS_BUILD.md](WINDOWS_BUILD.md)
