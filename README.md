...
- **Источник энтропии**: Операционная система (/dev/urandom на Unix, CryptGenRandom на Windows через OpenSSL)
- **Безопасность**: Криптографически стойкий генератор, подходящий для генерации ключей и IV
- **Использование**: Автоматическая генерация ключей при шифровании без `--key` (требование Sprint 3 - RNG-3)
- **Соответствие требованиям**: Используется системный CSPRNG (OS-provided CSPRNG) как требуется в Sprint 3

**Примечание**: Модуль `mouse_entropy.c` существует в проекте, но не используется для генерации ключей по умолчанию. Генерация ключей всегда использует системный CSPRNG через OpenSSL RAND_bytes().
