...
- Вывод в формате: `HMAC_ЗНАЧЕНИЕ ПУТЬ_К_ФАЙЛУ` (один пробел между значением и путем)
...
AES-CMAC обеспечивает аутентификацию и целостность данных с использованием AES-128.

```bash
cryptocore dgst --cmac --key КЛЮЧ --input ФАЙЛ [--verify ФАЙЛ_С_CMAC] [--output ВЫХОДНОЙ_ФАЙЛ]
```

- `--cmac`: Включает режим AES-CMAC (требует `--key`)
- `--key КЛЮЧ`: Ключ в виде 32-символьной шестнадцатеричной строки (16 байт для AES-128, обязателен при `--cmac`)
- `--verify ФАЙЛ`: Проверяет CMAC файла против значения в указанном файле
- Вывод в формате: `CMAC_ЗНАЧЕНИЕ ПУТЬ_К_ФАЙЛУ` (32 hex символа, один пробел между значением и путем)
...
# Генерация AES-CMAC и сохранение в файл
...
#### Конструкция HMAC и свойства безопасности

HMAC (Hash-based Message Authentication Code) реализован согласно RFC 2104 и использует SHA-256 из Sprint 4 как базовую хеш-функцию.
...
где:
- `H` — функция SHA-256
- `K` — ключ (произвольной длины)
- `m` — сообщение
- `opad` — внешний пад (0x5c, повторенный 64 раза)
- `ipad` — внутренний пад (0x36, повторенный 64 раза)
- `⊕` — операция XOR
- `∥` — конкатенация

**Обработка ключа:**
- Если длина ключа > 64 байт (размер блока SHA-256): ключ сначала хешируется с помощью SHA-256
- Если длина ключа < 64 байт: ключ дополняется нулями до 64 байт
- Результат всегда имеет размер 64 байта

**Свойства безопасности:**
- **Аутентификация**: HMAC гарантирует, что сообщение было создано обладателем секретного ключа
- **Целостность**: Любое изменение данных или использование неверного ключа приведет к ошибке верификации
- **Стойкость**: Безопасность HMAC основана на криптографической стойкости базовой хеш-функции (SHA-256)
- **Потоковая обработка**: Реализация обрабатывает файлы чанками (4096 байт), обеспечивая константное потребление памяти даже для больших файлов

#### Конструкция AES-CMAC и свойства безопасности

AES-CMAC (Cipher-based Message Authentication Code) реализован согласно NIST SP 800-38B и использует AES-128 из предыдущих спринтов.

**Алгоритм AES-CMAC:**
1. **Генерация подключа**: 
   - L = AES-Encrypt(0^128, K)
   - K1 = (L << 1) XOR (Rb если MSB(L) = 1), где Rb = 0x87 для 128-битных блоков
   - K2 = (K1 << 1) XOR (Rb если MSB(K1) = 1)
2. **CBC-MAC**: Вычисление CBC-MAC для всех блоков, кроме последнего
3. **Финальный блок**:
   - Если последний блок полный (16 байт): XOR с K1, затем шифрование
   - Если последний блок неполный: дополнение (0x80, затем нули), XOR с K2, затем шифрование

**Свойства безопасности:**
- **Аутентификация**: AES-CMAC гарантирует, что сообщение было создано обладателем секретного ключа
- **Целостность**: Любое изменение данных или использование неверного ключа приведет к ошибке верификации
- **Стойкость**: Безопасность AES-CMAC основана на криптографической стойкости AES-128
- **Потоковая обработка**: Реализация обрабатывает файлы чанками (4096 байт), обеспечивая константное потребление памяти
- **Стандарт**: Соответствует NIST SP 800-38B, используется в различных протоколах (IEEE 802.11i, TLS, IPsec)
...
│   │   └── cmac.c         # AES-CMAC (NIST SP 800-38B)
