...
| RNG-3 | Использовать vetted источник ОС/библиотеки | ✅ Выполнено | OpenSSL `RAND_bytes()` |
...
### Измененные файлы
1. `main.c` – Автогенерация ключа при шифровании без `--key`; интеграция CSPRNG для IV; предупреждения о слабых ключах
...
4. `README.md` – обновлено поведение CLI, примеры, раздел о CSPRNG, NIST STS
...
- Выделенный модуль CSPRNG
- Автогенерация ключа при шифровании (при отсутствии `--key`)
- Печать ключа в stdout (1 раз, hex)
- Интеграция CSPRNG в IV и ключи
- Обновление документации и тестов (уникальность/NIST STS)
- Сохранение совместимости и поведения Sprint 2
...
Реализован CSPRNG на базе OpenSSL, обновлен CLI для безопасной автогенерации ключей, обеспечена корректная интеграция с существующей логикой IV и сохранена совместимость с OpenSSL и требованиями предыдущих спринтов.
